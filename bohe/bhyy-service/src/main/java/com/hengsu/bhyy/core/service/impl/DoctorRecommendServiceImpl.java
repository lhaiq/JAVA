...
public class DoctorRecommendServiceImpl implements DoctorRecommendService {
    ...
    public List<Map<String, Object>> selectRecommends(Long presenter) {
        ...
        String sql = "SELECT dc.id,d.id as doctorId,d.real_name as realName,d.phone,d.hospital_name as hospitalName,dc.rank ,\n" +
    ...
    @Transactional
    @Override
    public void addRecommends(Long presenter, List<Long> presentees) {
        String sql = "delete from doctor_recommend where presenter = ?";
        jdbcTemplate.update(sql, presenter);

        //推荐列表
        if (CollectionUtils.isNotEmpty(presentees)) {
            for (int i = 0; i < presentees.size(); i++) {
                DoctorRecommendModel recommendModel = new DoctorRecommendModel();
                recommendModel.setPresenter(presenter);
                recommendModel.setPresentee(presentees.get(i));
                recommendModel.setUpdateTime(new Date());
                recommendModel.setRank(i);
                createSelective(recommendModel);
            }
